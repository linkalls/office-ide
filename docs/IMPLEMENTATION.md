...
The planner currently recognizes average-unit-price, tax-included-sales, and high-sales-highlight requests in English or Japanese. It derives the last populated row from the active Workbook IR, writes formula columns, or scans C-column values against thresholds expressed as yen, ten-thousands of yen, or `k`. Highlight proposals style only matching A:F rows; the sample `売上50万円以上を強調して` request finds four records and emits 24 operations. Unsupported or zero-match prompts produce suggestions and no operations. This is deliberately labeled `Local planner`: it proves review, data inspection, attribution, operation, and rollback semantics without pretending that a model or CLI process is connected.

The 38.2-second Agent README demo records every input character rather than injecting finished strings. A 48×58px pointer travels to every target with a visible trail, amber target frame, click ripple, and Japanese step caption. After Undo and Redo it travels back to G1 so the changed result is visually explicit. The recorder asserts that the pointer actually reached every target, verifies `=ROUND(10/3,2)` evaluates to `3.33`, Enter moves to G18, proposal/apply/history/Undo/Redo/recovery all succeed, and then decodes the complete MP4 with ffmpeg before accepting the artifact. The same browser run checks zero warning/error output and zero body overflow at desktop and compact viewports. Run it with `bun run demo:record`.
